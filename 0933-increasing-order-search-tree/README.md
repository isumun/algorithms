<h2><a href="https://leetcode.com/problems/increasing-order-search-tree">933. Increasing Order Search Tree</a></h2><p>Given the <code>root</code> of a binary search tree, rearrange the tree in <strong>in-order</strong> so that the leftmost node in the tree is now the root of the tree, and every node has no left child and only one right child.</p>
